...
async function payReferralCommission(user, amount, sourceOrOpts = 'ad') {
  if (!user || !user.referredBy) return;
  try {
    let opts = {};
    if (typeof sourceOrOpts === 'string') opts = { source: sourceOrOpts };
    else if (typeof sourceOrOpts === 'object') opts = sourceOrOpts;
    // Delegate commission logic to Referral.addCommission which handles transactions and logging
    const commission = await Referral.addCommission(user.firebaseUid, amount, opts);
